...
-- (using exports['std-ferriswheel']:getState / carEntity / insideGondola) and fires the intents
...
CreateThread(function()
    ...
    while true do
        ...
        if pos and boothPos and #(pos - boothPos) < (Bridge.boardRange or 40.0) then
            local s = exports['std-ferriswheel']:getState(TW)
            ...
            if s and s.boardable then
                ...
                if not mySeat then
                    ...
                    if car and exports['std-ferriswheel']:insideGondola(TW, car, pos.x, pos.y, pos.z)
                       and (now - lastIntent) > 400 then
                        lastIntent = now
                        TriggerServerEvent('std-ferriswheel:board', TW)
                    end
                elseif s.platformCar == mySeat.car then
                    -- our gondola is back at the platform, stopped: walk OUT -> release the seat
                    if not exports['std-ferriswheel']:insideGondola(TW, mySeat.car, pos.x, pos.y, pos.z)
                       and (now - lastIntent) > 400 then
                        lastIntent = now
                        TriggerServerEvent('std-ferriswheel:alight')
                    end
                end
            end
        end
